...
// testing  add 1channel support
__global__ void levels_image_kernel_1channel(float *image, float *rand, int batch, int w, int h, int train, float saturation, float exposure, float translate, float scale, float shift)
{
	int size = batch * w * h;
	int id = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
	if (id >= size) return;
...
extern "C" void forward_crop_layer_gpu(crop_layer layer, network_state state)
{
	...
	//printf("size1=%d\n", size);//debuging 一通道size=32*256*256=2097152，三通道size=32*256*256=2097152
	if (layer.c == 1)levels_image_kernel_1channel << <cuda_gridsize(size), BLOCK >> >(state.input, layer.rand_gpu, layer.batch, layer.w, layer.h, state.train, layer.saturation, layer.exposure, translate, scale, layer.shift);//testing add 1channel support
	else levels_image_kernel << <cuda_gridsize(size), BLOCK >> >(state.input, layer.rand_gpu, layer.batch, layer.w, layer.h, state.train, layer.saturation, layer.exposure, translate, scale, layer.shift);
	check_error(cudaPeekAtLastError());

    size = layer.batch*layer.c*layer.out_w*layer.out_h;
	//printf("size2=%d\n", size);//debuging 一通道size=32*1*256*256=2097152，三通道size=32*3*256*256=6291456
