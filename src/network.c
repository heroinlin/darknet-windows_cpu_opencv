...
float train_network_datum(network net, float *x, float *y)
{
    ...
    *net.seen += net.batch;
    ...
    if(((*net.seen)/net.batch)%net.subdivisions == 0) update_network(net);//每net.bacth*net.subdivisions=bacth=64(cfg文件中的batch值)次，更新一次网络
    return error;
}
...
float train_network(network net, data d)
{
    ...
    int batch = net.batch;//batch=net.batch=batch/subdivisions(cfg文件中的数值，parse_net_options()函数中)=64/2=32
	//printf("test   %d\n", batch);//debuging
	int n = d.X.rows / batch;//n=64/32=2,即n=subdivisions=2
    float *X = calloc(batch*d.X.cols, sizeof(float));
    float *y = calloc(batch*d.y.cols, sizeof(float));
    int i;
    float sum = 0;
