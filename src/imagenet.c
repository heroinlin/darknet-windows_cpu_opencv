...
void train_imagenet(char *cfgfile, char *weightfile)
{
    ...
    char *backup_directory = "D:/Dataset/train_last";
    printf("%s\n", base);
    ...
    int imgs = 1024;
    char **labels = get_labels("D:/Dataset/train_last/labels.txt");
    list *plist = get_paths("D:/Dataset/train_last/train.txt");
...
void test_imagenet(char *cfgfile, char *weightfile, char *filename)
{
    ...
    while(1){
        ...
        top_predictions(net, 2, indexes);//获取预测概率最大的前10个索引
        printf("%s: Predicted in %f seconds.\n", input, sec(clock()-time));
        for(i = 0; i < 2; ++i){
            int index = indexes[i];
            printf("%s: %f\n", names[index], predictions[index]);//依次输出预测类别和对应的概率
        }
        free_image(im);
        if (filename) break;
    }
}
void test_list_imagenet(char *cfgfile, char *weightfile, char *filename, char*savefile)
{
	network net = parse_network_cfg(cfgfile);
	if (weightfile){
		load_weights(&net, weightfile);
	...
	FILE *fp;
	char buff1[1024];
	snprintf(buff1, 1024, "%s/sort_result.txt", savefile);
	fp = fopen(buff1, "w");
	for (i = 0; i < m; i++){
		if (paths[i]){
...
void run_imagenet(int argc, char **argv)
{
    ...
    char *cfg = argv[3];
    char *weights = (argc > 3) ? argv[4] : 0;
	char *path1 = (argc > 4) ? argv[5] : 0;
	char *path2 = (argc > 5) ? argv[6] : 0;
	char *filename = (argc > 6) ? argv[7] : 0;
    if(0==strcmp(argv[2], "test")) test_imagenet(cfg, weights, filename);
	else if (0 == strcmp(argv[2], "test_list")) test_list_imagenet(cfg, weights, path1, path2);
