...
data load_data_region_gray(int n, char **paths, int m, int w, int h, int size, int classes, float jitter)
{
	char **random_paths = get_random_paths(paths, n, m);//m代表train.txt中包含的路径总个数
	int i;
	data d;
	d.shallow = 0;

	d.X.rows = n;//每行表示图片张数为n
	d.X.vals = calloc(d.X.rows, sizeof(float*));
	d.X.cols = h*w * 1;//每列代表每张图的矩阵变换为h*w*1的向量的长度

	int k = size*size*(5 + classes);//7*7*(5+1)=294
	d.y = make_matrix(n, k);//64*294的矩阵
	for (i = 0; i < n; ++i){
		//处理n张图
		image orig = load_image(random_paths[i], 0, 0, 1);
		//orig = grayscale_image(orig);//testing
		//printf("random_paths[%d]=%s\n", i,random_paths[i]);//debuging
		int oh = orig.h;
		int ow = orig.w;
		//原图像尺寸oh=288，ow=320
		//printf("oh=%d,ow=%d",oh,ow);//debuging
		int dw = (ow*jitter);
		int dh = (oh*jitter);
		...
		int ptop = rand_uniform(-dh, dh);
		int pbot = rand_uniform(-dh, dh);
		//printf("pleft=%d,pright=%d,ptop=%d,pbot=%d\n", pleft,pright, ptop , pbot);//debuging
		int swidth = ow - pleft - pright;
		int sheight = oh - ptop - pbot;

		float sx = (float)swidth / ow;
		float sy = (float)sheight / oh;
		//printf("swidth=%d,sheight=%d,sx=%d,sy=%d\n", swidth, sheight, sx, sy);//debuging
		int flip = rand_r(&data_seed) % 2;
		image cropped = crop_image(orig, pleft, ptop, swidth, sheight);
...
void *load_thread(void *ptr)
{
	...
	}else if (a.type == REGION_DATA_GRAY){
		*a.d = load_data_region_gray(a.n, a.paths, a.m, a.w, a.h, a.num_boxes, a.classes, a.jitter);
...
void get_next_batch(data d, int n, int offset, float *X, float *y)//以offset为间隔获取之后的n个数据
{
    int j;
    for(j = 0; j < n; ++j){
        int index = offset + j;
        memcpy(X+j*d.X.cols, d.X.vals[index], d.X.cols*sizeof(float));//X+j*d.X.cols表示第j张图的首地址
        memcpy(y+j*d.y.cols, d.y.vals[index], d.y.cols*sizeof(float));
    }
}
